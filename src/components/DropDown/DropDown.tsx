...
const DropDown = ({
  ...
  return (
    <div
      ref={menuRef}
      className={`relative text-asphalt text-sm capitalize 
      
      ${
        customStyle
          ? customStyle
          : ` ${isRounded ? "py-[8px] px-[16px]" : "p-3 pl-4"}  ${
              isRounded ? "rounded-full" : "rounded-lg"
            }   
      ${
        isRounded ? "bg-cloud" : different ? "bg-white" : "bg-cloud lg:bg-white"
      } `
      }
     

      ${!showSelected && !notFw && "lg:w-fit"}
      ${!isRounded && !notFw && "w-full "}
      ...
      <div
        ...
        className={`w-full cursor-pointer flex items-center justify-between ${
          !showSelected && "lg:justify-start"
        } lg:gap-8`}>
        <p className="flex items-center">
          {Icon && <Icon />}
          {!isRounded && (
            <>
              {t(data.label)} <span className=" block px-1">:</span>
            </>
          )}
          <span
